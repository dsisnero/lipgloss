...
## Summary
`Lipgloss::WrapWriter` needs to decode ANSI SGR (`CSI ... m`) and OSC-8 hyperlinks while preserving style state.
In `ultraviolet`, the expected helper APIs were missing/inaccessible for this use case, which forced local fallback parsing in `src/wrap.cr`.
...
2. We had to duplicate style parsing logic (`apply_sgr`) in Lipgloss, which increases drift risk vs Ultraviolet behavior.
3. Hyperlink/style decode behavior is now split across libraries instead of centralized in Ultraviolet.
...
3. Add shard-level tests that exercise downstream-style incremental ANSI parsing.
